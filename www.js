    ...
    if (pname in players) {
        ...
        if (player == answer | guesses.length == 8) {
            ...
            if (guesses.length == 8 && player != answer){
                document.getElementById('exampleModalLabel').innerHTML = `Nice try, but not there yet! The answer to today's Fencordle is ${answer[0]}.`
            }
            else {
                document.getElementById('exampleModalLabel').innerHTML = `Well played, you guessed today's Fencordle!`               
            }
